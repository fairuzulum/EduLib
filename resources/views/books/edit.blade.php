...
<body class="min-h-screen flex items-center justify-center bg-cover bg-center" style="background-image: url('{{ asset('images/shelf-books.jpg') }}');">
    <div class="container mx-auto px-6 py-8 bg-white bg-opacity-40 rounded-3xl shadow-xl max-w-2xl backdrop-blur-lg">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-lg">
            <h2 class="text-2xl font-bold text-gray-800 text-center mb-6">Edit Book Details</h2>
            <form action="{{ route('book.update', ['id' => $book->id]) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label for="title" class="block text-gray-700 font-semibold">Title</label>
                    <input type="text" name="title" id="title" value="{{ old('title', $book->title) }}" placeholder="Enter book title"
                ...
                <div>
                    ...
                    <input type="text" name="author" id="author" value="{{ old('author', $book->author) }}" placeholder="Enter author name"
                        class="w-full px-4 py-3 mt-2 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-gray-700 shadow-sm" required>
                </div>
                ...
                <div>
                    ...
                    <select name="genre" id="genre"
                        ...
                        <option value="Historical" {{ old('genre', $book->genre) == 'Historical' ? 'selected' : '' }}>Historical</option>
                    </select>
                </div>

                <div>
                    <label for="publisher" class="block text-gray-700 font-semibold">Publisher</label>
                    <input type="text" name="publisher" id="publisher" value="{{ old('publisher', $book->publisher) }}" placeholder="Enter publisher"
                        class="w-full px-4 py-3 mt-2 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-gray-700 shadow-sm" required>
                </div>

                <div>
                    <label for="published_year" class="block text-gray-700 font-semibold">Published Year</label>
                    <input type="number" name="published_year" id="published_year" value="{{ old('published_year', $book->published_year) }}" placeholder="Year published" min="1000" max="{{ date('Y') + 1 }}"
                        class="w-full px-4 py-3 mt-2 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-gray-700 shadow-sm" required>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" placeholder="Describe the book..."
                        class="w-full px-4 py-3 mt-2 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-gray-700 shadow-sm" required>{{ old('description', $book->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <div id="image-preview" class="mt-4 flex justify-center">
                        @if ($book->cover_image)
                            <img src="{{ asset('storage/' . $book->cover_image) }}" alt="Current Book Cover" class="w-48 h-64 object-cover rounded-lg shadow-md">
                        @endif
                    </div>
                </div>
